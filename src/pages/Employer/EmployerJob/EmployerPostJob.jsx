...
const EmployerPostJob = () => {
  ...
  //     if (response.ok) {
  //       console.log("roles testing", response?.data);
    ...
    try {
      ...
      while (nextPage) {
        ...
        if (response.ok) {
          ...
          allRoles = [...allRoles, ...data.results];
          nextPage = data.next;
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.start_date) formErrors.start_date = "Start Time is required";
    if (!formData.end_date) formErrors.end_date = "End Time is required";
    ...
    if (formData.salary > 10000000)
      formErrors.salary = "Please Enter Valid Salary Range";
    if (!formData.department) formErrors.department = "Department is required";
    // if (!formData.start_time) formErrors.starTime = "Start Time Is Required";
    // if (!formData.end_time) formErrors.endTime = "End Time Is Required";
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      try {
        setLoading(true);
        const response = await apiClient.post(`/facility/api/job/`, formData);
        ...
        console.log("Error while posting job data", error);
        setLoading(false);
      }
    } else {
  ...
  return (
    <Box>
      <Box className="px-sm-5 p-4">
        <Typography className="fw-bold">Post a Job</Typography>
      </Box>
      <Box
